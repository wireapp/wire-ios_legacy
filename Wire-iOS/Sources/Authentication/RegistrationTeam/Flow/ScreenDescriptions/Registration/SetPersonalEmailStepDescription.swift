...
class SetPersonalEmailStepSecondaryView: TeamCreationSecondaryViewDescription {
    ...
    init() {
        ...
        let usePhoneNumberButton = ButtonDescription(title: "registration.register_by_phone".localized, accessibilityIdentifier: "register_by_phone_button")
        self.views = [usePhoneNumberButton]

        usePhoneNumberButton.buttonTapped = { [weak self] in
            self?.actioner?.executeAction(.switchCredentialsType(.phone))
        }
...
final class SetPersonalEmailStepDescription: TeamCreationStepDescription {
    ...
    init() {
        ...
        subtext = "registration.email.subheadline".localized
        secondaryView = SetPersonalEmailStepSecondaryView()
    }
