...
extension ConversationInputBarViewController {
    ...
    func createInputBar() {
        ...
        inputBar = InputBar(buttons: [photoButton, mentionButton, sketchButton, gifButton, audioButton, pingButton, uploadFileButton, locationButton, videoButton])
        inputBar.translatesAutoresizingMaskIntoConstraints = false
        inputBar.textView.delegate = self
        registerForTextFieldSelectionChange()
        view.addSubview(inputBar)

        if let superview = inputBar.superview {
            let constraints = [
                inputBar.leadingAnchor.constraint(equalTo: superview.leadingAnchor),
                inputBar.topAnchor.constraint(equalTo: superview.topAnchor),
                inputBar.trailingAnchor.constraint(equalTo: superview.trailingAnchor)
            ]
            NSLayoutConstraint.activate(constraints) ///TODO: check it is broken?

            let bottomConstraint = inputBar.bottomAnchor.constraint(equalTo: superview.bottomAnchor)
