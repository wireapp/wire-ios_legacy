    ...
    override open var keyCommands: [UIKeyCommand]? {
        ...
        if inputBar.isEditing {
            commands.append(UIKeyCommand(input: UIKeyCommand.inputEscape, modifierFlags: [], action: #selector(escapePressed), discoverabilityTitle: "conversation.input_bar.shortcut.cancel_editing_message".localized))
                ...
                commands.append(UIKeyCommand(input: UIKeyCommand.inputUpArrow /*/TODO: if mention list exists, select the upper item */, modifierFlags: [], action: #selector(upArrowPressed), discoverabilityTitle: "conversation.input_bar.shortcut.edit_last_message".localized))
        }
        return commands
    }
}
