...
class CallEventHandler: CallEventHandlerProtocol {
    ...
    func reportIncomingVoIPCall(_ payload: [String: Any]) {
        ...
        DebugLogger.addStep(step: "ReportCallEvent protocol", eventID: "!")
        
        DispatchQueue.main.async(execute: {() -> Void in
            CXProvider.reportNewIncomingVoIPPushPayload(payload) { error in
                DebugLogger.addStep(step: "ReportCallEvent completion: ", eventID: "\(error)")
                if let error = error {
                    DebugLogger.addStep(step: "Failed to report callEvent: \(error) ", eventID: "!")
                    // TODO: handle
                }
            }
        })
...
public class LegacyNotificationService: UNNotificationServiceExtension, NotificationSessionDelegate {
    ...
    public func reportCallEvent(_ event: ZMUpdateEvent, currentTimestamp: TimeInterval) {
        guard
            ...
            DebugLogger.addStep(step: "Did not ReportCallEvent ", eventID: "!")
            return
        }

      DebugLogger.addStep(step: "ReportCallEvent ", eventID: "!")
